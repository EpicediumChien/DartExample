...
namespace DDPM.SA.Plugins.CMAManager
{
    ...
    public class CMAManagerPlugin : BaseAgentPlugin, IDisposableObservable, ICMAManagerSA, IRemoteManagement
    {
        ...
        public Task<RemoteManagementResult> Info(RemoteRequestArgs request)
        {
            ...
            {
                if (_CliManagerPlugin.checkDefer(DeferControlPanel.SRC_FROM_CMA, uniqueAgentGuid.ToString(), request.remote_request))
                {
        ...
        private void checkDeferSchedule(List<string> list)
        {
            ...
            foreach (string item in deferlist)
            {
                ...
                {
                    isRuncommand = !(_CliManagerPlugin.checkDeferSchedule(data.commandfrom, data.guid, data));
                }
            ...
            /*foreach (string i in removeItems) {
                DeferControlPanel.removeItem(i);
            }*/
            WriteLog($"[CMA] checkDeferSchedule::removeItems({counter})");

            //DeferControlPanel.removeItems(counter);
