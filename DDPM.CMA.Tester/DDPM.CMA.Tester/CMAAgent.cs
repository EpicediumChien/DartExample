...
namespace DDPM.CMA.Tester
{
    public class CMAAgent
    {
        ...
        private async void runRequest(string[] args)
        {
            ...
            // string jsonGetDisplayMulti = await ReadJsonFileAsync("GetDisplayMulti");
            ...
            while (isRunning)
            {
                ...
                if (Regex.IsMatch(input, pattern))
                {
                    ...
                    JsonDocument jsonDocument = JsonDocument.Parse(cmaRequest.remote_request);
                    foreach (var item in jsonDocument.RootElement.GetProperty("req").EnumerateArray())
                    {
                        requestNumList.Add(item.GetProperty("tid").GetInt32());
                    }
